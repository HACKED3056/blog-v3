...
备用地址：[网络安全: 科协ss0t网络安全学习资料分享仓库 - Gitee.com](https://gitee.com/ASUS_HACKED/cybersecurity/tree/比赛附件/其他附件)



首先我们打开流量分析就发现了这个问题,流量包太大了，Wireshark无法解析出来，我们需要使用修复工具对这个包进行重构

![image-20260309132859274](https://img2024.cnblogs.com/blog/3726946/202603/3726946-20260309141810675-2015068651.png)

这个网站是修复pcap文件的，我们使用它修复一下
[pcapfix - online pcap / pcapng repair service](https://f00l.de/hacking/pcapfix.php)

![image-20260309133631536](https://img2024.cnblogs.com/blog/3726946/202603/3726946-20260309141810102-45066459.png)

![image-20260309133842700](https://img2024.cnblogs.com/blog/3726946/202603/3726946-20260309141809475-881222256.png)
...
>
> - **找 Web 攻击：** 搜索常见的攻击关键字。
>   - `http contains "select"` 或 `http contains "union"`（SQL 注入）。
>   - `http contains "etc/passwd"`（任意文件读取）。
>   - `http.request.method == "POST"`（查看上传的文件或 Webshell 的交互）。
> - **找敏感行为：** * `tcp.flags.reset == 1`（可能有端口扫描）。
...
> - **文件传输：** 如果发现流量中有 `PK` (Zip)、`JFIF` (JPG)、`PNG` 等文件头。
>   - 使用 **"Export Objects -> HTTP"** 直接提取。
>   - 或者手动提取 Hex 原始数据并还原。
> - **数据导出：** 有时 Flag 隐藏在大量重复的数据包中（如 ICMP 数据位、TCP 的 ID 字段、TTL 字段），这时需要写 Python 脚本配合 `scapy` 库进行批量提取。
>
> ------
>
> 4. 常见“坑点”与 Flag 位置
...
>
> 1. **非预期协议：** 比如 Flag 被藏在 `ICMP` 的 Data 字段中，每包一个字节，拼起来就是 Flag。
> 2. **USB 流量：** 键盘（Keyboard）或鼠标（Mouse）流量。需要提取坐标或按键码，通过脚本绘图或映射成文字。
> 3. **TLS 解密：** 题目可能会提供一个 `.key` 文件或 SSL KEYLOG 文件，需要配置 Wireshark 进行解密才能看到内容。
> 4. **隐写术：** 导出的图片、压缩包本身可能还有层隐写（Outguess, Steghide, 暴力破解等）。

-----

![image-20260309134323687](https://img2024.cnblogs.com/blog/3726946/202603/3726946-20260309141808832-2001685925.png)
...
- **异常提醒：** 如果发现某个平时很小的协议（比如 ICMP 或 DNS）字节数异常大，那极大概率是**隐蔽隧道传输**。
...
- **HTTP (Hypertext Transfer Protocol)：** 截图中显示 HTTP 占了约 **15.7%** 的字节。
  ...
  - **子项分析：** 注意看它下面的 `Line-based text data` 和 `JavaScript Object Notation (JSON)`，这代表流量里有纯文本和 JSON 数据。
...
![image-20260309135503614](https://img2024.cnblogs.com/blog/3726946/202603/3726946-20260309141808212-1405019241.png)

哎呀啥也没有,那直接搜一下看看会不会出flag

![image-20260309135931685](https://img2024.cnblogs.com/blog/3726946/202603/3726946-20260309141807533-1051743026.png)

哎？从1159到1184字段都出现了flag信息，

难不成想告诉我们flag就在这里？

![image-20260309140133943](https://img2024.cnblogs.com/blog/3726946/202603/3726946-20260309141806725-32650392.png)

看到右边的Hex编码,出现了一个类似flag的倒影 lf 字符,看看下一条是不是也是这样？

![image-20260309140247098](https://img2024.cnblogs.com/blog/3726946/202603/3726946-20260309141806089-958519732.png)

那我们的猜想就是，从编号1159到1184中的Identificantion中都是整个flag的分组倒影，每两个字符为一个倒影

![image-20260309141508794](https://img2024.cnblogs.com/blog/3726946/202603/3726946-20260309141716012-1671185650.png)
